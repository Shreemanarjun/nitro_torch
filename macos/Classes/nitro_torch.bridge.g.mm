...
uint8_t* nitro_torch_max_level(NitroError* _nitro_err) {
    if (_nitro_err) { _nitro_err->hasError = 0; }  // S8: clear slot
    JNIEnv* env = GetEnv();
    if (env == nullptr) { return nullptr; }
    jmethodID methodId = g_mid_maxLevel_call;
    if (methodId == nullptr) { LOGE("Method not found: maxLevel_call sig=()[B"); return nullptr; }
...
JNIEXPORT void JNICALL Java_nitro_nitro_1torch_1module_NitroTorchJniBridge_initialize(JNIEnv* env, jobject thiz, jclass localClass) {
    ...
    if (g_bridgeClass != nullptr) {
        ...
        g_mid_maxLevel_call = env->GetStaticMethodID(g_bridgeClass, "maxLevel_call", "()[B");
        if (!g_mid_maxLevel_call && env->ExceptionCheck()) { env->ExceptionClear(); LOGE("Method not found: maxLevel_call sig=()[B"); }
...
extern uint8_t* _nitro_torch_call_maxLevel(void);
uint8_t* nitro_torch_max_level(NitroError* _nitro_err) {
    if (_nitro_err) { _nitro_err->hasError = 0; }
#ifdef __OBJC__
    @try {
